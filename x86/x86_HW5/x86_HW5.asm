...
; Set offsets of Interrupt Descriptor 
	mov eax, ISR_00
	mov word [idt0+0], ax		; Offset Low of ISR_00
	shr eax, 16
	mov word [idt0+6], ax		; Offset High of ISR_00

	mov eax, ISR_13
	mov word [idt13+0], ax		; Offset Low of ISR_13
	shr eax, 16
	mov word [idt13+6], ax		; Offset High of ISR_13

	mov eax, ISR_80
	mov word [idt80+0], ax		; Offset Low of ISR_80
	shr eax, 16
	mov word [idt80+6], ax		; Offset High of ISR_80
...
ISR_00:
	...
	mov dword [esp], Task1_Return	; EIP of TSS1
	iret
...
ISR_13: ; General Protection Fault
	...
	add esp, 4						; Error Code of TSS2
	mov dword [esp], Task2_Return	; EIP of TSS2
	iret
...
ISR_80:	; User Defined ISR
	...
	mov dword [esp], Task2_Return2	; EIP of TSS2
	iret
...
idt0:				; Divide Error Exception
	dw	0000h		; ISR address
	dw	SYS_EXT_SEL	; segment selector
	db	00h			; reserved
	db	8Eh			; p=1, dpl=0, d=1
	dw	0000h		; ISR address

; idt01 ~ idt12
times	8*12 db 0	; reserved

idt13: 				; General Protection Fault
	dw	0000h		; ISR address
	dw	SYS_EXT_SEL	; segment selector
	db	00h			; reserved
	db	8Eh			; p=1, dpl=0, d=1
...
; idt14 ~ idt47
times	8*34 db 0	; reserved

idt48: 					; Task Gate Descriptor
	dw	0000h			; ISR address
	dw	TSS2Selector	; segment selector
	db	00h				; reserved
	db	85h				; p=1, dpl=0, d=1
	dw	0000h			; ISR address

; idt49 ~ idt79
times	8*31 db 0	; reserved
